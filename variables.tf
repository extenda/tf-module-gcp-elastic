variable "elastic_project_gcp_secret" {
  description = "GCP project ID which contains secret for Elastic Cloud credentials"
  type        = string
  default     = "tf-admin-90301274"
}

variable "elastic_api_key" {
  description = "Name of the secret that contains Elastic Cloud API key to extract from Secret Manager"
  type        = string
...
variable "enable_kibana" {
  description = "Whether to deploy Kibana or not"
  type        = bool
  default     = false
}
